...
if __name__ == "__main__":
    ...
    assert TEST in ['non_inferiority', 'Equivalence']
    ...
    for scenario in scenarios_df.index:
        inferiority_pval[scenario] = {'less': [], 'greater': []}
        ...
        for _ in range(0, n_trials):
            ...
            if TEST == 'non_inferiority':
                # one sample t test
                p_value_less = ttest_1samp(a=differences,
                                                 popmean=scenarios_df.loc[scenario, 'effect_size'],
                                                 alternative='less').pvalue
                inferiority_pval[scenario]['less'].append(p_value_less)

                p_value_greater = ttest_1samp(a=differences,
                                              popmean=scenarios_df.loc[scenario, 'effect_size'],
                                              alternative='greater').pvalue
                inferiority_pval[scenario]['greater'].append(p_value_greater)

            elif TEST == 'Equivalence':
                # two sample t test
                p_value_two_sided = ttest_1samp(a=differences,
                            popmean=scenarios_df.loc[scenario, 'effect_size'],
                            alternative='two-sided').pvalue
                two_sided_pvals[scenario].append(p_value_two_sided)
